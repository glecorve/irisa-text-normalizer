 ...
 * - bzip2 requires that bzip2 is availaible via command line
...
public class CRFFeatureExporter<IN extends CoreMap> {
   ...
   */
  private String ubPrefixFeatureString(String feat)
  {
    if (feat.endsWith("|C")) {
      return "U-" + feat;
  ...
  public void printFeatures(String exportFile, int[][][][] docsData, int[][] labels)  {
    try {
      PrintWriter pw = FileUtils.getPrintWriter(exportFile);
  ...
  public static void main(String[] args) throws Exception {
    ...
    Collection<List<CoreLabel>> docs = crf.makeObjectBankFromFile(inputFile);
