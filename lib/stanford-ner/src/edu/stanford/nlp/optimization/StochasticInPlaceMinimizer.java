 ...
 * @author Angel Chang
 */
public class StochasticInPlaceMinimizer<T extends Function> implements Minimizer<Function>, HasEvaluators {
  protected double xscale,xnorm;
  protected double[] x;
  protected int t0;  // Initial stochastic iteration count
  protected double sigma = 1,lambda;
  ...
  {
    if (LogPrior.LogPriorType.QUADRATIC.equals(prior.getType())) {
      sigma = prior.getSigma();
    } else {
      throw new RuntimeException("Unsupport prior type " + prior.getType());
    }
    if (numPasses >= 0) {
      this.numPasses = numPasses;
    } else {
