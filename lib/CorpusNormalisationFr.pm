...
my $DATE_PREFIX = "(en|durant|pendant|depuis|avant|après|jusqu'à|vers|de|à)";

my $ALLOWED_TAGS = "(CURRENCY|DATE|LOCATION|ORGANIZATION|QUANTITY|PERSON|TIME|URL)";
my $NUMBER = "[0-9]+(?:[\.,\/][0-9]+)*";
my $LITTERAL_DIGIT = "(?:un|deux|trois|quatre|cinq|six|sept|huit|neuf)";
my $LITTERAL_TEENS = "(?:dix|onze|douze|treize|quatorze|quinze|seize|dix-sept|dix-huit|dix-neuf)";
my $LITTERAL_NTY = "(?:vingt|trente|quarante|cinquante|soixante|soixante-dix|quatre-vingts?|quatre-vingt-dix)";
my $LITTERAL_SMALL_NUMBER = "(?:$LITTERAL_NTY(?:-et)?-$LITTERAL_DIGIT|$LITTERAL_TEENS|$LITTERAL_DIGIT)";
my $LITTERAL_NUMBER = "(?:$LITTERAL_SMALL_NUMBER (?:milliard|million|mille)s?)*(?: et| )?(?:$LITTERAL_DIGIT centimes?)?$LITTERAL_SMALL_NUMBER(?: (?:point|virgule) $LITTERAL_SMALL_NUMBER)?";
...
# 	print "B\n";
	...
	elsif ($pos{$lcw} =~ /_(DET|PRO|P|CJ)/) {
# 	print "C\n";
		$retour = 1;
	}
	# Will Smith -> 0
	# Will they -> 1
	elsif (defined($lexicon{$lcw}) && $pos{$w} =~ /_(NP|SYM)/ && $pos{$w} !~ /_(N|ADJ)(?=_|$)/) {
# 	print "D\n";
		if ($x =~ /^[a-z]/) { return 1; }
		else { return 0; }
	}
...
#		print STDERR "s/(^| |\b)$ereg[$i] ?([0-9]+(?:[\.,\/][0-9\-]+)*) *([bmM]?)n?\.?( |\n|$)/$1$3 $4 $2$5/gm\n";
		$$p_text =~ s/(^| |\b)$ereg[$i] ?([0-9]+(?:[\.,\/][0-9\-]+)*) *([mM]?)(?:(?:illio)?n?)\.?( |\n|$)/$1$3 $4 $2$5/gm;
...
sub year2text {
        ...
        if ($year =~ /^(1[1-9])(..)$/) {
        	if (rand() < 0.5) {
	        	$year = number_to_fr($1)." cent ".number_to_fr($2);
        	}
        	else {
	        	$year = number_to_fr($year);
        	}
        }
        else {
	        $year = number_to_fr ( $year );
...
sub expand_ad_bc {
	my $x = shift;
	$x =~ /^A\.?D\.?$/ && return "après Jésus-Christ";
	$x =~ /^B\.?C\.?$/ && return "avant Jésus-Christ";
	return "";
}
...
sub rewrite_time {
	...
	# normal process
			...
			if (rand() < 0.5) {
				if ($minute == 30) {
						$ret = "$hour_str heure".plural($hour)." et demie";
				}
				elsif ($minute == 15) {
						$ret = "$hour_str heure".plural($hour)." et quart";
				}
				elsif ($minute == 45) {
						$ret = number_to_fr(($hour+1) % 12)." heure".plural($hour)." moins le quart";
				}
					elsif ($minute > 30) {
						$ret = number_to_fr(($hour+1) % 12)." heure".plural($hour)." moins ".(number_to_fr(60-$minute));
				}
				else {
		...
		#AM/PM
			...
			if (uc $ampm eq "PM") {
				if ($hour > 6) {
					$ampm_str .= (rand() < 0.67?" du soir":"");
				}
				else {
					$ampm_str .= (rand() < 0.67?" de l'après-midi":"");
				}
			} elsif (uc $ampm eq "AM") {
					$ampm_str .= (rand() < 0.5?" du matin":"");
			}
		    }


		if ($second != 0) {
			$ret = $hour_str." ".number_to_fr($minute)." et ".$second_str;
		}
		else {
			$ret .= $ampm_str;
		}
...
sub date_and_time {
	...
	sub rewrite_duration {
		...
		my $s_str = "";
		my $ms_str = "";
		$d_str = "$days jour".plural($days) unless ($days == 0);
		$h_str = "$hours heure".plural($hours) unless ($hours == 0);
		$m_str = "$minutes minute".plural($minutes) unless ($minutes == 0);
		$s_str = "$seconds seconde".plural($seconds) unless ($seconds == 0);
		$ms_str = "$milliseconds milliseconde".plural($milliseconds) unless ($milliseconds == 0);
...
#	$text =~ s/(^| )$DAY,? *([0-3]?[0-9](?:th|rd|st)?)($END_SEP)/"TOTOOOOO"/eigm;
	$$p_text =~ s/(^| )$DAY,? *((?:le)?) *([0-3][0-9]),? *$MONTH(?: ,)? *((?:\d{1,4})?)($END_SEP)/"$1 <DATE> $2 $3 $4 $5 $6 ".year2text($7)." <\/DATE>$8"/eigm;
...
#	$$P_TEXT =~ s/(\d(?:[ ,\.](?:\d+))+)([A-ZÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝa-zàáâãäåçèéêëìíîïñòóôõöøùúûüý])/$1 $2/gm;
	...
	while ($$P_TEXT =~ s/(^| )($NUMBER) *[xX] *($NUMBER)(?=\/| |$)/$1$2 par $3 $4/g) { }
#	$$P_TEXT =~ s/ × (\d+)/ multiplied by $1/gm;
	$$P_TEXT =~ s/ × / fois /gm;
	$$P_TEXT =~ s/ <= (\d+)/ est inférieur ou égal à $1/gm;
	$$P_TEXT =~ s/ >= (\d+)/ est supérieur ou égal à $1/gm;
	$$P_TEXT =~ s/ < (\d+)/ est inférieur à $1/gm;
	$$P_TEXT =~ s/ <(\d+)/ inférieur à $1/gm;
	...
	$$P_TEXT =~ s/(^|\b)3\/4(\b|$)/$1."trois quarts".$3/gem;
...
# 	$$P_TEXT =~ s/(^|\b)(\d{1,2})(ers?)(\b|$)/$1.process_er(number_to_fr($2)).$3.$4/gem;
	...
	$$P_TEXT =~ s/(^|\b)1 ?er(s?)(\b|$)/$1."premier".$2.$3/gem;
	$$P_TEXT =~ s/(^|\b)1 ?(ère|ere|re)(s?)(\b|$)/$1."première".$2.$3/gem;
...
sub complex_abbreviations {
	my $P_TEXT = shift;
	$$P_TEXT =~ s/(^| )([XVI]+\.?(?:e|è|eme|ème|ieme|ième))(?=$END_SEP)/$1.rom2dec($2,1)."e$3"/gem;
	$$P_TEXT =~ s/(^| )([XVI]+) ([Ss]iècle|[Aa]rmée|[Rr]épublique)(?=$END_SEP)/$1.rom2dec($2,1)."e $3$4"/gem;
	$$P_TEXT =~ s/(^|\b)([A-ZÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝ][a-zàáâãäåçèéêëìíîïñòóôõöøùúûüý]+)-([IVXLCDM]+)(?=$END_SEP)/$1.$2." ".rom2dec($3,1).$4/gem; # Bordeaux-I => Bordeaux 1
...
sub acronyms {
	...
	sub explode_acronym {
		...
		foreach my $t (split(/-/,$x)) {
			# WORD -> Word
			if (($t =~ /[aeiouy]/i && (defined($lexicon{lc($t)}) || defined($lexicon{ucfirst(lc($t))})) && length($t) >= 3) || ($t =~ /[aeiouy]/i && length($t) > 3)) {
				#nothing
			}
...
sub hyphenate {
	...
	sub suffixation {
		...
		my $b = shift;
		my $c = $a.$b;
		if ($c !~ /^(-t-elles?|-t-ils?|-t-on|-t-en|[rR]endez-vous|[eE]ntre-lui|[cC]hez-[mt]oi|[cC]hez-nous)$/) {
			return "$a $b";
		}
		else {
			return $c;
	...
	sub de_suffixation {
		...
		my $b = shift;
		my $c = $a.$b;
		if ($c !~ /^(-t-elles?|-t-ils?|-t-on|-t-en|-ce|-elles?|-ils?|-je|-la|-les?|-leur|-lui|-mêmes?|-m\'|-moi|-nous|-on|-toi|-tu|-t\'|-vous|-en|-y|-ci|-là)$/) {
			return "$a $b";
		}
		else {
			return $a.$b
...
# 	$$p_text =~ s/-/ /gm; #replace remaining dash with blanks
	$$p_text =~ s/ -t-ils(?= )/ -ils/gm; #"sont -t-ils" =>  "sont -ils"
	$$p_text =~ s/ -t-elles(?= )/ -elles/gm; # idem elles
		#tirets en début de phrase ou mot
	$$p_text =~ s/^-+([^0-9])/$1/gm;
	$$p_text =~ s/ -([A-ZÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝ][^ \-\.])/ $1/gm;
	$$p_text =~ s/ ([0-9]+)- / $1 /gm; #pas de tiret seul apres un nombre
	$$p_text =~ s/- t- (il|on|elle)/ -t-$2 /gm; #pas de tiret seul apres un nombre
...
sub apostrophes {
	...
	my $w = shift;
	if ($w !~ /^'/ && $w =~ /'/) {
	    #si le mot commence par une majscule, ne pas le couper
	    if ($w =~ /^[A-ZÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝ]/) {
		return $w;
	    }
	    #si le mots avec le tiret est dans le lexique, ne pas le couper
	    elsif (defined($lexicon{$w})) {
		return $w;
	    }
	    else {
		my @tab = split(/'/,$w);
		return (shift @tab)."' ".apo_couper_ou_pas(join("'",@tab));
	    }
	}
	else {
	    return $w;
    ...
    $$p_text =~ s/(^| |\n)([^ \n]+)('[^ \n]+)+(?=$| |\n)/$1.apo_couper_ou_pas($2.$3)/ge;
    $$p_text =~ s/ ([0-9]+)' / $1 /gm; #pas d'apostrophe seule apres un nombre
}
